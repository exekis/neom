...
export default function DAWPage() {
  ...
  // Stable onProjectLoad and options to prevent re-creating callbacks each render
  const onProjectLoad = useCallback((projectData: { tracks: AudioTrack[]; trackStates: any; masterVolume: number; name: string; }) => {
  ...
  const projectManager = useProjectManager(
    useMemo(() => ({ onProjectLoad }), [onProjectLoad])
  );
  ...
  // Destructure stable function to use in deps
  const { loadProject } = projectManager;
  ...
  useEffect(() => {
    ...
    const loadSessionAudio = async () => {
      ...
      if (projectId) {
        try {
          const audioContext = initAudioContext();
          await loadProject(projectId, audioContext);
          setIsFirstProject(false);
          setHasAutoSaved(true);
